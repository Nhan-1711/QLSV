...
// AI Chatbot Routes (Global)
Route::post('/chat/send', [App\Http\Controllers\ChatController::class, 'sendMessage'])->middleware(['auth'])->name('chat.send');
Route::get('/chat/widget', [App\Http\Controllers\ChatController::class, 'index'])->middleware(['auth'])->name('chat.widget');
...
Route::middleware(['auth', 'role:admin'])->prefix('admin')->name('admin.')->group(function () {
    ...
    Route::get('course_classes/{courseClass}/attendance/{session}/statistics', [App\Http\Controllers\Lecturer\AttendanceController::class, 'sessionStatistics'])->name('attendance.session_statistics');
...
// AI Chatbot Routes
    ...
    // Notifications
    Route::get('/course_classes', [App\Http\Controllers\Student\CourseClassController::class, 'index'])->name('course_classes.index');
    Route::get('/course_classes/{id}', [App\Http\Controllers\Student\CourseClassController::class, 'show'])->name('course_classes.show');
...
Route::middleware(['auth', 'role:admin'])->prefix('admin')->name('admin.')->group(function () {
    ...
    Route::get('course_classes/{classId}/evaluate', [App\Http\Controllers\Student\EvaluationController::class, 'create'])->name('student.evaluations.create');
    Route::post('course_classes/{classId}/evaluate', [App\Http\Controllers\Student\EvaluationController::class, 'store'])->name('student.evaluations.store');

    Route::get('course_classes/{classId}/evaluations', [App\Http\Controllers\Lecturer\EvaluationController::class, 'index'])->name('lecturer.evaluations.index');
    

Route::middleware(['auth', 'role:lecturer'])->prefix('lecturer')->group(function () {
    Route::get('/evaluations_dashboard', [App\Http\Controllers\Lecturer\EvaluationController::class, 'dashboard'])->name('lecturer.evaluations.dashboard');
});

Route::middleware(['auth', 'role:lecturer'])->prefix('lecturer')->group(function () {
    Route::post('classes/{courseClass}/grades/lock', [App\Http\Controllers\Lecturer\GradeController::class, 'lock'])->name('lecturer.grades.lock');
});