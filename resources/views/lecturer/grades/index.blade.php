...
@section('content')
    <div class="card mb-4">
        <div class="card-body">
            ...
            <fieldset {{ $courseClass->is_grades_locked ? 'disabled' : '' }}>
                @csrf
                <table class="table table-bordered table-hover">
                    ...
                    <tbody>
                        ...
                        @endforeach
                                ...
                                @if(!$courseClass->is_grades_locked)
                    <button type="submit" class="btn btn-primary">Lưu bảng điểm</button>
                    <button type="button" onclick="confirmLock()" class="btn btn-danger ms-2">Chốt điểm</button>
                @else
                    <button type="button" class="btn btn-secondary" disabled><i class="fas fa-lock me-1"></i> Bảng điểm đã chốt</button>
                @endif
                        </fieldset>
</form>
    ...
    <form id="lock-grades-form" action="{{ route('lecturer.grades.lock', $courseClass) }}" method="POST" class="d-none">
        @csrf
    </form>

    <script>
        function confirmLock() {
            if(confirm('Bạn có chắc chắn muốn chốt điểm? Sau khi chốt sẽ không thể chỉnh sửa điểm số của lớp này nữa.')) {
                document.getElementById('lock-grades-form').submit();
            }
        }
    </script>
@endsection
