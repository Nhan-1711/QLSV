                            ...
                            <td>
                                <a href="{{ route('lecturer.attendance.show_session', ['courseClass' => $courseClass->id, 'session' => $session->id]) }}"
                                    class="btn btn-sm btn-info"><i class="fas fa-eye"></i> Chi tiết / Điểm danh</a>
                                        <a href="{{ route('lecturer.attendance.session_statistics', ['courseClass' => $courseClass->id, 'session' => $session->id]) }}" class="btn btn-sm btn-primary ms-1"><i class="fas fa-chart-pie"></i> Thống kê</a>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
